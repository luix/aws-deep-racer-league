...
cppolygon, = plt.plot(ctrlpts[:, 0], ctrlpts[:, 1], color='sandybrown', linestyle='-.', marker='o', markersize='3')  # control points polygon
curveplt, = plt.plot(curvepts[:, 0], curvepts[:, 1], color='lawngreen', linestyle='-')  # evaluated curve points

tanline = plt.quiver(ctarr[:, 0, 0], ctarr[:, 0, 1], ctarr[:, 1, 0], ctarr[:, 1, 1], color='red', angles='xy', scale_units='xy', scale=1, width=0.003)  # tangents
